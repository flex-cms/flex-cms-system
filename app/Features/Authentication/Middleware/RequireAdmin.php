...
final readonly class RequireAdmin implements MiddlewareInterface
{
    ...
    {
        if (!$this->auth->check()) { return $request->expectsJson() ? Response::json(['status' => 'error', 'message' => 'Unauthenticated.'], 401) : Response::redirect($this->loginUrls->loginUrl()); }
        if (!$this->auth->isAdmin()) { return $request->expectsJson() ? Response::json(['status' => 'error', 'message' => 'Forbidden.'], 403) : Response::html('<h1>403 - Forbidden</h1>', 403); }
