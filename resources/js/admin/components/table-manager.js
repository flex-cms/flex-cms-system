...
export default (config = {}) => ({
    ...
    confirmDeleteMessage:
        config.confirmDeleteMessage || "Сигурни ли сте, че искате да изтриете този елемент?",
    confirmRestoreMessage:
        config.confirmRestoreMessage || "Сигурни ли сте, че искате да възстановите този елемент?",
    ...
    errorForceDeleteMessage:
        config.errorForceDeleteMessage || "Грешка при перманентното изтриване.",
    ...
    successForceDeleteMessage:
        config.successForceDeleteMessage || "Записът беше изтрит перманентно!",
    ...
    init() {
        const tbody =
            this.$el.querySelector('tbody[x-data*="sortable"]') || this.$el.querySelector("tbody");
    ...
    async deleteItem(id) {
        ...
        try {
            ...
            if (res.data && res.data.success) {
                notify(res.data.message || this.successDeleteMessage, "success", () =>
                    window.removeRowWithAnimation(this.$el),
                );
            } else {
    ...
    async restoreItem(id) {
        ...
        try {
            ...
            if (res.data && res.data.success) {
                notify(res.data.message || this.successRestoreMessage, "success", () =>
                    window.removeRowWithAnimation(this.$el),
                );
            } else {
    ...
    async forceDeleteItem(id) {
        ...
        try {
            ...
            if (res.data && res.data.success) {
                notify(res.data.message || this.successForceDeleteMessage, "success", () =>
                    window.removeRowWithAnimation(this.$el),
                );
            } else {
